-- Strategic schema remediation script (Phase 1 + Phase 2)
-- Safe to run multiple times on PostgreSQL.
-- Purpose:
-- 1) Ensure strategic tables, indexes, and FK constraints exist.
-- 2) Optionally stamp baseline migration history when expected tables exist.
...
BEGIN
    ...
    IF missing_count = 0 THEN
        ...
        VALUES ('20260304164301_InitialSchemaBaseline', '9.0.0')
        ON CONFLICT ("MigrationId") DO NOTHING;
    ELSE
        RAISE NOTICE 'Skipped baseline migration history stamp because % required tables are missing.', missing_count;
    END IF;
END $$;
